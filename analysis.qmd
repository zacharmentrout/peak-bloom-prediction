...
daily_forecast_temp_init$id<- paste(daily_forecast_temp_init$date, daily_forecast_temp_init$location, daily_forecast_temp_init$model, sep = "_")  # Create a composite key
daily_forecast_temp <- daily_forecast_temp_init[order(daily_forecast_temp_init$id, daily_forecast_temp_init$timestamp_run, decreasing = TRUE), ][!duplicated(daily_forecast_temp_init$id), ]
...
mod_data <- util$prep_mod_data(pheno_data_init = d_init, temp_data_init = daily_temp,  forecast_temp_data_init = daily_forecast_temp_means, plot_temp = F)
```
